  ...
  module GraphqlIntegration
    module Matchers
      ...
      module MatchGraphqlResponse
        ...
        ##
        # This error is thrown when a variable is missing that is required for the
        # GraphQL response matcher.
        ...
        matcher :match_graphql_response do |_expected| # rubocop:disable Metrics/BlockLength
          ...
          def check_variables!
            REQUIRED_TEST_VARIABLES.each do |variable_name, example_value|
              raise TestVariableMissing.new(variable_name, example_value) unless defined?(send(variable_name))
            end
          end
        ...
        def load_response(dir, filename, variables = {})
          ...
          variables.each do |key, value|
            json_file.gsub!("\"{{#{key}}}\"", JSON.dump(value))
          end
